    ...
    <main style={{ minHeight: '100vh', background: '#FAF7F2', fontFamily: 'sans-serif' }}>
      ...
      <section style={{ maxWidth: '600px', margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '1rem', flexWrap: 'wrap' }}>
          {['', '수능', '토익', '공무원', '자격증', '대학원'].map(tag => (
            <span key={tag} onClick={() => { setFilterTag(tag); setTimeout(fetchPosts, 0) }} style={{ fontSize: '12px', padding: '5px 12px', borderRadius: '14px', cursor: 'pointer', background: filterTag === tag ? '#C9A882' : '#fff', color: filterTag === tag ? '#fff' : '#9A8A78', border: '0.5px solid #E8E0D4' }}>
              {tag ? `#${tag}` : '전체'}
            </span>
          ))}
        </div>
          ...
          <div style={{ background: '#fff', borderRadius: '12px', border: '0.5px solid #E8E0D4', padding: '1.25rem', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: '8px' }}>
              ...
              <input
                ...
                placeholder="#태그 추가"
                style={{ border: 'none', outline: 'none', fontSize: '12px', color: '#9A8A78', background: 'transparent', width: '80px' }}
              />
            </div>
